...
for game in video_game_sales:
    if game[GLOBAL_SALES] > 25:
        print(f"{game[NAME]}: {game[GLOBAL_SALES]}M copies sold globally")
...
if total_na_sales > total_jp_sales:
    print(f"\nNorth America had higher sales by ${total_na_sales - total_jp_sales:.2f}M")
elif total_jp_sales > total_na_sales:
    print(f"\nJapan had higher sales by ${total_jp_sales - total_na_sales:.2f}M")
else:
    print("\nBoth regions had equal sales!")
...
print(f"\nNintendo published {len(nintendo_games)} out of {len(video_game_sales)} games in the dataset.")
